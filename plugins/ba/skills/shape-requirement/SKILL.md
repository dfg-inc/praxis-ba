...
Once the whole batch of a CR's impact entries is shaped, run the `ba-lint` skill before `prepare-wp`: `praxis-ba validate` covers canon structure **and** body semantics (a `[NEEDS CLARIFICATION]` marker left in an active requirement, a reference to an NFR id absent from the project catalogue, a tag outside the vocabulary, child ahead of parent, broken relative links) — and `prepare-wp`'s link-integrity gate will fail on exactly those broken links.
